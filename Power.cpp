...
Return<int32_t> Power::getFeature(SlimFeature feature)  {
    if (feature == SlimFeature::SUPPORTED_PROFILES) {
...
status_t Power::registerAsSystemService() {
    ...
    ret = ISlimPower::registerAsService();
    if (ret != 0) {
        ALOGE("Failed to register ISlimPower (%d)", ret);
        goto fail;
    } else {
        ALOGI("Successfully registered ISlimPower");
    }
